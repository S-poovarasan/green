...
interface AddSyllabusProps {
    ...
    editSyllabusPopup: boolean;
    setEditSyllabusPopup: React.Dispatch<React.SetStateAction<boolean>>;
    editSyllabusData: string;
    setEditSyllabusData: React.Dispatch<React.SetStateAction<string>>;
    editSyllabusId: number;
}

const EditSyllabus: React.FC<AddSyllabusProps> = (props) => {
    ...
    const AddSyllabus = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const res = await SyllabusService.updateSyllabus(props.editSyllabusData, props.editSyllabusId);
            if (res.status) {

                props.setEditSyllabusPopup(false);
                props.setEditSyllabusData("");
    ...
    return (
        <Dialog
            ...
            visible={props.editSyllabusPopup}
            ...
            onHide={() => props.setEditSyllabusPopup(false)}
        >
            <form onSubmit={AddSyllabus}>
                <div className="w-full">
                    <div
                        onClick={() => props.setEditSyllabusPopup(false)}
                ...
                <div className="overflow-y-auto custom-scroll flex flex-col gap-3 px-3 pb-2 mt-4">
                    <TextInputWithLabel
                        ...
                        bgColor="#00808054"
                        value={props.editSyllabusData}
                        onChange={(e) => {
                            props.setEditSyllabusData(e.target.value)
                            setError({
                                status: false,
                                message: "",
                            });
