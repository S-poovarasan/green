...
interface RecordingLinkSessionManagementProps {
    updateRecordTopicPopup: boolean;
    setUpdateRecordTopicPopup: React.Dispatch<React.SetStateAction<boolean>>;
    updateRecordTopicDetails: UpdateLinkTopic;
    setUpdateRecordTopicDetails: React.Dispatch<React.SetStateAction<UpdateLinkTopic>>;
    GetSessionData: () => void;
}

const RecordingLinkSessionManagement: React.FC<RecordingLinkSessionManagementProps> = (props) => {
    ...
    const handleSubmitUpdateMeetingLink = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        try {
            const response = await SessionManagementService.updateMeetingRecordLink(props.updateRecordTopicDetails);
            if (response.status) {
                props.setUpdateRecordTopicPopup(false);
    ...
    return (
        <Dialog
            ...
            visible={props.updateRecordTopicPopup}
            ...
            onHide={() => props.setUpdateRecordTopicPopup(false)}
        >
            <form onSubmit={handleSubmitUpdateMeetingLink}>
                <div className="w-full">
                    <div
                        onClick={() => props.setUpdateRecordTopicPopup(false)}
                    ...
                    <div className="w-full flex gap-2 items-end">
                        <label className=" w-full font-semibold text-gray-700 mb-2 block">
                            <TextInputWithLabel
                                ...
                                name={`topic-from`}
                                className='w-[100%]'
                                label={`Meeting Link for ${props.updateRecordTopicDetails.refCLName}`}
                                placeholder={`Enter Meeting Link`}
                                bgColor="#00808054"
                                value={props.updateRecordTopicDetails.refCLRecordingLink}
                                onChange={(e) => {
                                    props.setUpdateRecordTopicDetails(prev => ({
                                        ...prev,
                                        refCLRecordingLink: e.target.value
                                    }));
                                }}
                                required
                            />
                        ...
                        <ButtonsLabel
                            ...
                            onClick={async () => {
                                try {
                                    ...
                                    props.setUpdateRecordTopicDetails(prev => ({
                                        ...prev,
                                        refCLRecordingLink: text   // Set clipboard text
                                    }));
                                } catch (err) {
...
export default RecordingLinkSessionManagement;